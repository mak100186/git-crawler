...
public class AggregateTrendsCommandHandlerTests(PostgresFixture fixture) : PostgresTestBase(fixture)
{
    ...
    public async Task Handle_RepositoryWithScoreAndSummary_IsCountedInItsCategory()
    {
        ...
        DbContext.Scores.Add(NewScore(repository.Id, 50, _timeProvider.GetUtcNow()));
        ...
        var trend = await DbContext.TrendAggregates.SingleAsync();
    ...
    public async Task Handle_RepositoryWithOnlyScore_NoSummaryYet_IsExcluded()
    {
        ...
        Assert.Empty(await DbContext.TrendAggregates.ToListAsync());
    }
    ...
    public async Task Handle_MultipleRepositoriesInSameCategory_ComputesRepositoryCountAndAverageScore()
    {
        ...
        DbContext.Repositories.AddRange(repositoryA, repositoryB);
        await DbContext.SaveChangesAsync();

        DbContext.Scores.Add(NewScore(repositoryA.Id, 40, _timeProvider.GetUtcNow()));
        DbContext.Scores.Add(NewScore(repositoryB.Id, 60, _timeProvider.GetUtcNow()));
        DbContext.Summaries.Add(NewSummary(repositoryA.Id));
        DbContext.Summaries.Add(NewSummary(repositoryB.Id));
        await DbContext.SaveChangesAsync();
        ...
        var trend = await DbContext.TrendAggregates.SingleAsync(t => t.Category == "C#");
    ...
    public async Task Handle_RepositoryWithMultipleScoreRows_UsesLatestByComputedAtUtc_NotHighestEverValue()
    {
        ...
        DbContext.Scores.Add(NewScore(repository.Id, 90, _timeProvider.GetUtcNow().AddDays(-2)));
        DbContext.Scores.Add(NewScore(repository.Id, 30, _timeProvider.GetUtcNow().AddDays(-1)));
        DbContext.Summaries.Add(NewSummary(repository.Id));
        await DbContext.SaveChangesAsync();
    ...
    public async Task Handle_RerunForSamePeriod_UpsertsExistingTrendAggregateRow_DoesNotCreateDuplicate()
    {
        ...
        DbContext.Repositories.Add(repositoryA);
        await DbContext.SaveChangesAsync();
        ...
        Assert.Equal(1, await DbContext.TrendAggregates.CountAsync());
        var firstRun = await DbContext.TrendAggregates.SingleAsync();
        ...
        DbContext.Repositories.Add(repositoryB);
        await DbContext.SaveChangesAsync();
        DbContext.Scores.Add(NewScore(repositoryB.Id, 60, _timeProvider.GetUtcNow()));
        DbContext.Summaries.Add(NewSummary(repositoryB.Id));
        await DbContext.SaveChangesAsync();
        ...
        Assert.Equal(1, await DbContext.TrendAggregates.CountAsync());
        var secondRun = await DbContext.TrendAggregates.SingleAsync();
    ...
    public async Task Handle_TwoDifferentCategoriesInSamePeriod_EachGetsOwnTrendAggregateRow()
    {
        ...
        DbContext.Scores.Add(NewScore(repositoryA.Id, 40, _timeProvider.GetUtcNow()));
        DbContext.Scores.Add(NewScore(repositoryB.Id, 70, _timeProvider.GetUtcNow()));
        DbContext.Summaries.Add(NewSummary(repositoryA.Id));
        DbContext.Summaries.Add(NewSummary(repositoryB.Id));
        await DbContext.SaveChangesAsync();
        ...
        var csharpTrend = await DbContext.TrendAggregates.SingleAsync(t => t.Category == "C#");
        Assert.Equal(1, csharpTrend.RepositoryCount);
        Assert.Equal(40, csharpTrend.AverageScore);

        var pythonTrend = await DbContext.TrendAggregates.SingleAsync(t => t.Category == "Python");
