...
- **Optimize for signal over popularity** — the core design principle: if GitHub Trending shows what's already successful, this platform should identify what is _about to become_ successful.

## Non-Goals

Explicitly out of scope for this version (v1 / MVP) — grouped by _why_ they're excluded, not just what they are:

**Deferred future enhancements — on the roadmap, just not v1:**

- Personalized discovery via user-defined interest profiles (Goal 4 in the seed concept) — out of scope for v1 only: v1 ships discovery/scoring with fixed, global criteria, because per-user interest weighting needs the base engine proven first. **That precondition is met — Q3 resolved 2026-09-08 and this is now committed Tranche v2 scope**, pending `idea-triage` for its phase and feature IDs. The three bullets below it remain deferred.
...
- As a user, I want to browse the repository catalog through a single, focused Hidden Gems view so that I don't have to reconcile two near-identical lists, with each hidden gem's own trend — its own score history, not a blended figure shared by every repo in its language — shown right on its card.
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- | --------- |
| Q1  | Cloud LLM (Azure OpenAI/OpenAI) vs. local/self-hosted model for AI summarization — cost/quality tradeoff, given the provider-agnostic constraint above. **Settled for now, not closed (2026-09-08): local/self-hosted via LM Studio, per ADR-016 — that is what the code does and what it will keep doing.** A cloud provider stays aspirational with no cost/quality evidence gathered and no trigger to revisit; the provider-agnostic abstraction is what keeps the door open. Reopen only if a concrete driver appears (summary quality shortfall, or host hardware becoming the bottleneck). | Maxx  | Deferred  |
...
| Q3  | Should personalized discovery (Goal 4 / user-defined interests) be scoped into an early post-MVP phase, or is it a longer-term future enhancement with no near-term commitment? **Resolved 2026-09-08: early phase — committed to Tranche v2.** Enters the backlog via normal triage; phase number and feature IDs assigned there. | Maxx  | Yes       |
...
| v3      | 2026-09-08 | Q3 resolved: personalized discovery (Goal 4) committed to Tranche v2 as an early phase rather than a long-term enhancement. Closes PMBook PM-002. | User decision |
| v4      | 2026-09-08 | Q1 restated as Deferred rather than open: local LM Studio (ADR-016) is the standing decision; cloud LLM is aspirational with no trigger to revisit. Non-Goals annotated so personalized discovery no longer reads as deferred. | User decision |
...
| v8      | 2026-08-04 | US-8 corrected, not narrowed this time: "each hidden gem's own category trend" was wrong on its face — v6 described the trend chip as computed per the repo's _category_ (language), meaning every repo sharing a language showed the identical growth figure, not a value specific to that repo. Reworded to "own trend... not a blended figure shared by every repo in its language" now that the implementation computes it from the repository's own Score history instead (Architecture v22, `docs/project-management.md` v28) | Operator: "Trend is currently calculated per language. I want it to be calculated per repository and then shown. What is currently being shown is the trend aggregate for the topic?" |
