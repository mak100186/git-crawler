...
// local copies. Only the typed InvokeAsync<T>(object, ...) is exercised by GenerateSummariesJob (see
// that class's own comment for why it's the typed overload, not the bare object one); every other
