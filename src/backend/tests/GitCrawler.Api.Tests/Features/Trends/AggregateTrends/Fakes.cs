...
// features' own local copies. Only InvokeAsync(object, ...) is exercised by AggregateTrendsJob;
// every other member of this fairly large interface throws if called, so a test would fail loudly
// instead of silently passing on the wrong overload.
